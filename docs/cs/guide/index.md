...
S každou vlastností Audiovisual Core je spojena její hodnota. Datový typ této hodnoty je rovněž specifikován v normativním dokumentu. Datové typy mohou zahrnovat volný text, konkrétní literály převzaté z kontrolovaného slovníku specifikovaného v normativním dokumentu nebo řadu dalších datových typů specifikovaných a popsaných v normativním dokumentu. In the case of a
